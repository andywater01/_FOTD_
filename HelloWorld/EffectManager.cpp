...
Selectable EffectManager::m_selectable2 = Selectable("AssignmentUI");
int check = 0;
...
void EffectManager::CreateTab()
{
	...
	if (ImGui::BeginTabItem(m_selectable2.GetName().c_str(), m_selectable2.GetSelected()))
	{
		temp = true;
		check = 2;
...
void EffectManager::CreateEditor()
{
	if (check == 1)
	{
		...
		if (ImGui::TreeNode("Pixelate Effect"))
		{
			if (m_pixelate != -1)
			{
				...
				if (ImGui::DragFloat("Pixel Size", &pixelSize, 0.1f, 1.f, 16.f))
				{
	...
	if (check == 2)
	{
		if (ImGui::Button("Set Animation 1"))
		{

			auto entity = EntityIdentifier::MainPlayer();
		...
		if (ImGui::Button("Set Animation 2"))
		{
		...
		auto entity = EntityIdentifier::MainPlayer();
		auto& position = ECS::GetComponent<Transform>(entity);
		
		float charXPosition = position.GetPositionX();
		
		
		//Change X-Position
		if (ImGui::DragFloat("X Position", &charXPosition, 0.1f, -50.f, 50.f))
		{
			ECS::GetComponent<Transform>(entity).SetPositionX(charXPosition);
			
		}

		float charYPosition = position.GetPositionY();

		//Change Y-Position
		if (ImGui::DragFloat("Y Position", &charYPosition, 0.1f, -100.f, 50.f))
		{
			ECS::GetComponent<Transform>(entity).SetPositionY(charYPosition);

		}

		auto entity2 = EntityIdentifier::MainCamera();
		auto& camPosition = ECS::GetComponent<Camera>(entity2);
		float charZRotation = camPosition.GetRotationAngleZ();

		//Change Rotation of Camera
		if (ImGui::SliderAngle("Camera Angle", &charZRotation, -360.f, 360.f))
		{
			ECS::GetComponent<Camera>(entity2).SetRotationAngleZ(charZRotation);
