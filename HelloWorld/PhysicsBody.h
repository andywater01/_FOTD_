...
public:
	...
	//Gets the max velocity for the each individual axis
	...
	//*if the offset is 0.0 then all the corners will be relative to the
	//center of the actual sprite
	vec2 GetCenterOffset() const;
	//Gets the corner of the body
	...
	//Set position (just sets the variable, doens't actually set the position)
	...
	//If other body type were ever implemented we couild set it here
...
private:
	...
	//The types of the bodies this body collides with
...
inline void from_json(const nlohmann::json& j, PhysicsBody& phys)
{
	...
	//Set does gravity affect the body?
